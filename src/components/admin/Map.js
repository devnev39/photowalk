...
export default function Map({
    ...
    const mapRef = useRef();
    const [mapLng, setMapLng] = useState(!plan.markers.length ? -121.91599 : plan.markers.map(l => l.lng).reduce((partialSum, a) => partialSum + a, 0) / plan.markers.length);
    const [mapLat, setMapLat] = useState(!plan.markers.length ? 37.36765 : plan.markers.map(l => l.lat).reduce((partialSum, a) => partialSum + a, 0) / plan.markers.length);
    ...
    useEffect(() => {
        ...
        let markers = [];
        map.on("click", (event) => {
            const marker = new tt.Marker().setLngLat(event.lngLat).setDraggable(true).addTo(map);
            const lngLat = {lng: event.lngLat.lng, lat: event.lngLat.lat};
            marker.on("dragstart", (marker_event) => {
                console.log("Clicked on marker !");
                marker.remove();
                markers = markers.filter(m => m != marker);
                setPlan(current => {return {...current, markers: current.markers.filter(m => JSON.stringify(m) != JSON.stringify(lngLat))}});
            })
            setPlan(current => {return {...current, markers: [...current.markers, lngLat]}});
            markers.push(marker);
        });
        if(!plan) return;

        plan.markers.forEach(marker => {
            const mkr = new tt.Marker().setLngLat([marker.lng, marker.lat]).setDraggable(true).addTo(map);
            markers.push(mkr);
            mkr.on("dragstart", (marker_event) => {
                console.log("Clicked on marker !");
                mkr.remove();
                markers = markers.filter(m => m != mkr);
                setPlan(current => {return {...current, markers: current.markers.filter(m => JSON.stringify(m) != JSON.stringify(marker))}});
            })
        })
    ...
    // Important! Always set the container height explicitly
        ...
        //     />
            // <div>
                <Box sx={{height: "50vh"}} id='mapDiv' ref={mapRef}>
